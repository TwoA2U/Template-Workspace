...
def normalize_template(template_id: str, template: dict, used_names: set[str]) -> dict:
    ...
    normalized["name"] = (
        normalized.get("name") or "Untitled Template"
    ).strip() or "Untitled Template"
...
def build_template_body(template_id: str, template: dict) -> dict:
    return {
        ...
        if isinstance(template.get("sections"), list)
        else [],
    }
...
def seed_template_dir() -> None:
    ...
    if (
        not BUNDLED_TEMPLATE_DIR.exists()
        or BUNDLED_TEMPLATE_DIR.resolve() == TEMPLATE_DIR.resolve()
    ):
        return
    ...
    with STATE_LOCK:
        for path in sorted(TEMPLATE_DIR.glob("*.json")):
            ...
            if not isinstance(template_id, str) or not template_id:
                if isinstance(data.get("name"), str) and isinstance(
                    data.get("sections"), list
                ):
            ...
            if template_id in templates:
                add_warning(
                    warnings, path, f'duplicate template id "{template_id}" ignored'
                )
                continue
...
def build_pdf(output_path: Path, title: str, markdown: str) -> None:
    ...
    for block in blocks:
        if block == "---":
            ...
            story.append(
                HRFlowable(width="100%", color=colors.HexColor("#e9e3db"), thickness=1)
            )
        ...
        if block.startswith("> "):
            ...
            story.append(
                Paragraph(
                    format_inline_markdown(quote_text).replace("\n", "<br/>"),
                    quote_style,
                )
            )
            continue
        story.append(
            Paragraph(format_inline_markdown(block).replace("\n", "<br/>"), body_style)
        )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Launch Template Workspace desktop app."
    )
    parser.add_argument(
        "-v",
        "--verbose",
